...
If the user click onto the specific card post in the Main Page, they are directed to the **Missing Pet Detail** Page. In this page, they can read the details information about the missing pet. The missing location is written in the address, but is also shown on the map below with the location mark. This way, people can easily check where the missing pet was lost.
...
If the users found the missing pet and want to delete the Missing Pet Post they registered, user can click the **Delete** button on top right corner of the Missing Pet Detail page. Then a message pops up, asking the user whether they really want to delete the Post. If the user select Yes, the Post gets deleted and the user is directed to the Main Page.
